...
def crop_video(sub_set, video, crop_path, instanc_size):
    ...
    for xml in xmls:
        ...
        for object_iter in objects:
            ...
            z, x = crop_like_SiamFC(im,
                                    bbox,
                                    instanc_size=instanc_size,
                                    padding=avg_chans)
            cv2.imwrite(
                join(video_crop_base_path,
                     '{:06d}.{:02d}.z.jpg'.format(int(filename), trackid)), z)
            cv2.imwrite(
                join(video_crop_base_path,
...
def main(instanc_size=511, num_threads=24):
    ...
    for sub_set in sub_sets:
        ...
        with futures.ProcessPoolExecutor(max_workers=num_threads) as executor:
            fs = [
                executor.submit(crop_video, sub_set, video, crop_path,
                                instanc_size) for video in videos
            ]
...
if __name__ == '__main__':
    ...
    print('Total complete in {:.0f}m {:.0f}s'.format(time_elapsed // 60,
                                                     time_elapsed % 60))
