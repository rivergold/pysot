...
parser.add_argument('--window-influence',
                    default='0.1,0.8,0.05',
                    type=parse_range)
                ...
                for lr in args.lr:
                    for ins in args.search_region:
                        ...
                        tracker_path = os.path.join(
                            benchmark_path, (model_name + '_r{}'.format(ins) +
                                             '_pk-{:.3f}'.format(pk) +
                                             '_wi-{:.3f}'.format(wi) +
                                             '_lr-{:.3f}'.format(lr)))
                        if 'VOT2016' == args.dataset or 'VOT2018' == args.dataset:
                            ...
                            if _check_and_occupation(video_path, result_path):
                                continue
                            pred_bboxes = run_tracker(tracker,
                                                      video.imgs,
                                                      video.gt_traj,
                                                      video.name,
                                                      restart=True)
                            ...
                            with open(result_path, 'w') as f:
                                for x in scores:
                                    f.write('\n') if x is None else f.write(
                                        "{:.6f}\n".format(x))
                            result_path = os.path.join(
                                video_path, '{}_time.txt'.format(video.name))
                            with open(result_path, 'w') as f:
                            ...
                            result_path = os.path.join(
                                tracker_path, '{}.txt'.format(video.name))
                            if _check_and_occupation(tracker_path,
                                                     result_path):
                                continue
                            pred_bboxes, _, _ = run_tracker(tracker,
                                                            video.imgs,
                                                            video.gt_traj,
                                                            video.name,
                                                            restart=False)
