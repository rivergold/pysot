...
class MaskCorr(DepthwiseXCorr):
    def __init__(self,
        ...
        super(MaskCorr, self).__init__(in_channels, hidden, out_channels,
...
class Refine(nn.Module):
    ...
    def forward(self, f, corr_feature, pos):
        p0 = F.pad(f[0], [16, 16, 16, 16])[:, :, 4 * pos[0]:4 * pos[0] +
                                           61, 4 * pos[1]:4 * pos[1] + 61]
        p1 = F.pad(f[1], [8, 8, 8, 8])[:, :, 2 * pos[0]:2 * pos[0] + 31, 2 *
                                       pos[1]:2 * pos[1] + 31]
        p2 = F.pad(f[2],
                   [4, 4, 4, 4])[:, :, pos[0]:pos[0] + 15, pos[1]:pos[1] + 15]
        ...
        out = self.post2(
            F.upsample(self.h0(out) + self.v0(p0), size=(127, 127)))
