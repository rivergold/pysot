...
class LogScheduler(LRScheduler):
    def __init__(self,
        ...
        self.end_lr = end_lr
        self.epochs = epochs
        self.lr_spaces = np.logspace(math.log10(start_lr), math.log10(end_lr),
                                     epochs)
...
def _build_lr_scheduler(optimizer, config, epochs=50, last_epoch=-1):
    return LRs[config.TYPE](optimizer,
...
def _build_warm_up_scheduler(optimizer, epochs=50, last_epoch=-1):
    ...
    sc1 = _build_lr_scheduler(optimizer, cfg.TRAIN.LR_WARMUP, warmup_epoch,
                              last_epoch)
    sc2 = _build_lr_scheduler(optimizer, cfg.TRAIN.LR, epochs - warmup_epoch,
                              last_epoch)
